...
foreach($result as $row){
	...
	if($description !== ""){
		...
		$description_link_i = array("\n", "« ", " »", "«", "»", '"');
		$description_link_o = array('', '“', '”', '“', '”', '&#34;');
		$description_link = str_replace($description_link_i, $description_link_o, $description_link);
...
<div class="content-home" style="padding-top: 15px; padding-bottom: 0px;" id="<?=$date;?>">
...
?>
		...
		<div style="margin-left: 74px; padding-top: 13px; line-height: 16px;">
			<a href="<?= WEBSITE_URL."/index.php?feed=".$feed_id;?>" style="color:var(--feedbot-title); font-weight: bold; text-decoration: none;"><?=truncate($feed_name,30);?></a>
<?php if($youtubeid !== ""){ ?>
			<img src="<?=WEBSITE_URL;?>/assets/youtube.png" alt="YouTube channel" style="width:20px; margin-left: 4px; margin-bottom: 1px; vertical-align: middle;">
<?php } ?>
<?php if($peertubeid !== NULL && $peertubeid !== ""){ ?>
			<img src="<?=WEBSITE_URL;?>/assets/peertube.png" alt="PeerTube channel" style="width:14px; margin-left: 5px; vertical-align: middle;">
<?php } ?>
			<br>
...
<?php if($caracters_counter > "400"){ ?>
	<div id="read-more-<?=$article_id;?>" style="width:100%; font-weight: bold; text-align: center; margin-top: 10px; cursor: pointer;">
		<?=READ_MORE;?>
	</div>
<?php } ?>
...
<?php } ?>
	...
	<div class="timeline-thumbnail" title="<?=$title;?>">
		<div class="timeline-thumbnail-content" style="background-image: url(<?=$thumbnail;?>);">
			<a href="<?php if ($youtubeid == "" && $peertubeid == "") { echo $url; } else { echo WEBSITE_URL ?>/index.php?watch=<?=$youtubeid.$peertubeid; } ?>" <?php if ($youtubeid == "" && $peertubeid == "") { ?> target="_blank" <?php } ?> style="display:block; width:100%; height:100%;"></a>
		</div>
