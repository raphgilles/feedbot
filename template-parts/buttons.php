...
<?php if ($is_shared !== "1") { ?>
		...
		<form class="timeline-buttons unshare_<?=$article_id;?>" style="display: none;">
			...
			<button onclick="unshare(<?=$article_id;?>);" type="submit" class="timeline-buttons" title="<?=SHARED;?> : <?=relativedate($date);?>" style="color:var(--feedbot-purple);">
...
<?php if ($bookmarked !== "1") { ?>
		...
		<form class="timeline-buttons unbookmark_<?=$article_id;?>" style="display:none;" class="timeline-buttons-div">
			...
			<input type="hidden" name="article_id" value="<?=$article_id;?>">
			<button onclick="unbookmark(<?=$article_id;?>);" class="timeline-buttons" title="<?=READ_LATER;?>" style="color:red;">
				<i class="fa fa-bookmark" aria-hidden="true" style="margin-right: 5px;"></i> <?=READ_LATER;?>
			</button>
		</form>
