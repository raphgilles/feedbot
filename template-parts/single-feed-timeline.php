...
$isconnected = $_SESSION['uid'];
    ...
    $embed = $row['embed'];
    $is_audio = pathinfo($embed, PATHINFO_EXTENSION);
    if ($is_audio == "mp3" OR $is_audio == "aac" OR $is_audio == "wav") { $is_audio = 1; }
...
<div class="content-home" style="padding-top: 15px; padding-bottom: 0px;" id="<?=$date;?>">
    ...
    <div class="timeline-thumbnail" title="<?=$title;?>" <?php if ($is_audio == 1){?> onclick="event.preventDefault(); podcast('<?=$article_id;?>')"<?php } ?>>
