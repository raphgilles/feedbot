...
$feed_thumbnail = str_replace("/var/www/my_webapp/www/storage/icons/", WEBSITE_URL."/storage/icons/", $feed_thumbnail);
...
<div style="background-image: url(<?=$thumbnail;?>)" class="videos-thumbnail" style="padding-top: 0; margin-top: 0;">
        <div class="videos-youtube-iframe"><iframe src="https://www.youtube-nocookie.com/embed/<?=$youtubeid;?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>
</div>
<?php } ?>
...
<div style="background-image: url(<?=$thumbnail;?>)" class="videos-thumbnail" style="padding-top: 0; margin-top: 0;">
        <div class="videos-youtube-iframe" style="z-index:1;"><iframe src="<?=$embed;?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>
</div>
...
<div class="video_item">
    ...
    <div style="height:80px;">
        <a href="<?=WEBSITE_URL."/feed/".$feed_id;?>"><img src="<?=$feed_thumbnail;?>"  style="border-radius:50%; aspect-ratio: 1/1; width:60px; float:left; margin-right:20px;" title="<?=$feed_name;?>" /></a>
        <div title="<?=$feed_name;?>" style="padding-top: 4px; line-height:16px;"><a href="<?=WEBSITE_URL."/feed/".$feed_id;?>" style="color:var(--feedbot-title); font-weight: bold; text-decoration: none;"><?=$feed_name;?></a> <?php if ($youtubeid !== NULL && $youtubeid !== "") { ?><img src="<?=WEBSITE_URL;?>/assets/youtube.png" alt="YouTube channel" style="width:20px; margin-left: 4px; margin-bottom: 4px; vertical-align: middle;" /><?php } ?><?php if ($peertubeid !== NULL && $peertubeid !== "") { ?><img src="<?=WEBSITE_URL;?>/assets/peertube.png" alt="PeerTube channel" style="width:16px; margin-left: 5px; vertical-align: bottom; margin-top: 4px;" /><?php } ?><br />
        <span style="font-size: 12px;"><?=$website;?><br /><?=$date;?></span>
        </div>
        ...
        <button onclick="subscribe(<?=$feed_id;?>)" type="submit" title="<?=SUBSCRIBE_TO;?> <?=$feed_name;?>"/><span><i class="fa fa-rss" aria-hidden="true" style="margin-right: 5px;"></i> <?=SUBSCRIBE;?></span></button>
        </form>
        ...
        <button onclick="unsubscribe(<?=$feed_id;?>)" type="submit" class="unsuscribe" title="<?=UNSUBSCRIBE_TO;?> <?=$feed_name;?>"/><span><i class="fa fa-rss" aria-hidden="true" style="margin-right: 5px;"></i> <?=UNSUBSCRIBE;?></span></button>
        </form>
...
<p style="white-space:pre-line;"><a href="<?=$theurl;?>" target="_blank" class="feed_item_title" title="<?=$title;?>"><?=$title;?></a></p>

    <div <?php if ($caracters_counter > "600") { ?>class="description-youtube-short"<?php } ?>  id="description">
    <?php if ($description !== "") { ?> <p style="margin-top:10px; word-break: break-word; white-space: pre-line;"><?=$description;?></p> <?php } ?>
    <p style="margin-top:18px; word-break: break-all; white-space:pre-line;"><a href="<?=$theurl;?>" target="_blank" title="<?=$title;?>"><?=$theurl ?></a></p>
    </div>
    <?php if ($caracters_counter > "600") { ?><div id="read-more" style="width:100%; font-weight: bold; text-align: center; margin-top: 10px; cursor: pointer;"><?=READ_MORE;?></div><?php } ?>
    <div style="height:20px;"></div>
        ...
        <div style="position:relative; display:block; height:28px;">
            ...
            <?php if ($isconnected == "") { ?>
                ...
                <button type="submit" class="timeline-buttons" title="<?=SHARE;?>"><i class="fa fa-retweet" aria-hidden="true" style="margin-right:5px;"></i> <?=SHARE;?></button>
                </form>
            <?php } elseif ($is_shared !== "1") { ?>
                ...
                <button onclick="unshare(<?=$article_id;?>)" type="submit" class="timeline-buttons" title="<?= SHARED ?> : <?=relativedate($date);?>" style="color:#563ACC;"><i class="fa fa-retweet" aria-hidden="true" style="margin-right:5px;"></i> <?=SHARED;?></button>
                </form>
            ...
            <?php if ($isconnected == "") { ?>
                ...
                <button type="submit" class="timeline-buttons" title="<?=READ_LATER;?>" style="margin-right:5px;"><i class="fa fa-bookmark" aria-hidden="true"></i> <?=READ_LATER;?></button>
                </form>
...
<h3 style="text-align:center; margin-bottom:14px;"><i class="fa fa-youtube-play" aria-hidden="true" style="margin-right:5px;"></i> <?=OTHER_VIDEOS;?></h3>
...
<?php
        ...
        $title_suggestions = str_replace( "-", ' ', $title_suggestions);
        $title_suggestions = str_replace( ":", ' ', $title_suggestions);
        $title_suggestions = str_replace( "_", ' ', $title_suggestions);
        $title_suggestions = str_replace( "+", ' ', $title_suggestions);
        $title_suggestions = str_replace( ",", ' ', $title_suggestions);
        $title_suggestions = str_replace( "’", '', $title_suggestions);
        $title_suggestions = str_replace( "|", ' ', $title_suggestions);
        ...
        $title_suggestions = str_replace( "#", '', $title_suggestions);
        $title_suggestions = str_replace( "\\", '', $title_suggestions);
        $title_suggestions = str_replace( "Λ", '', $title_suggestions);
        $title_suggestions = str_replace( "%", '', $title_suggestions);
        ...
        $sql = "SELECT * FROM articles WHERE (youtubeid IS NOT NULL OR peertubeid IS NOT NULL) AND (youtubeid != '' OR peertubeid != '') AND (feed_id LIKE '%$feed_id%' OR title REGEXP '[[:<:]]".$firstword."[[:>:]]' $other OR excerpt REGEXP '[[:<:]]".$firstword."[[:>:]]' $other2)";
        //echo $sql;
...
<h3 style="text-align:center; margin-bottom:14px;"><i class="fa fa-retweet" aria-hidden="true"></i> Partages</h3>
</div>
<?php
foreach ($result as $row) {
$status_url = $row['url'];
...
<div class="content-videos shares">
    <div style="height:50px; width: 100%;">
        ...
        <a href="<?=$status_origin;?>" style="color:var(--feedbot-gray); font-size:12px; margin-left: 5px; text-decoration:none;" target="_blank"><?=relativedate($status_date);?></a>
        </div>
        ...
        <div style="width:40px; aspect-ratio: 1 / 1; background-image: url('<?=$status_avatar;?>'); background-size: cover; background-position: center; border-radius: 6px; float:left;"><a href="<?=$status_userlink;?>" style="display:block; width:100%; height:100%;" target="_blank"></a></div>
            ...
            <span style="font-size:12px; color:var(--feedbot-gray);"><?=$status_aka;?></span>
            </div>
    </div>
