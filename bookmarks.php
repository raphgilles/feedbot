...
<div class="title"><i class="fa fa-bookmark" aria-hidden="true"></i> Vos marque-pages</div>
...
<div class="content-home" style="padding-top: 15px; padding-bottom: 0px;" id="<?=$article_id;?>">
    ...
    <div style="height:75px;">
        ...
        <div style="margin-left: 74px; padding-top: 13px; line-height: 16px;"><a href="<?= WEBSITE_URL."/index.php?feed=".$feed_id;?>" style="color:var(--feedbot-title); font-weight: bold; text-decoration: none;"><?=truncate($feed_name,30);?></a><?php if ($youtubeid !== "") { ?><img src="<?=WEBSITE_URL;?>/assets/youtube.png" alt="YouTube channel" style="width:20px; margin-left: 6px; margin-top: 1px; vertical-align: top;" /><?php } ?><?php if ($peertubeid !== NULL && $peertubeid !== "") { ?><img src="<?=WEBSITE_URL;?>/assets/peertube.png" alt="PeerTube channel" style="width:14px; margin-left: 4px; vertical-align: middle;" /><?php } ?><br />
    ...
    <?php if ($caracters_counter > "400") { ?><div id="read-more-<?=$article_id;?>" style="width:100%; font-weight: bold; text-align: center; margin-top: 10px; cursor: pointer;">Lire la suite</div><?php } ?>
    ...
    <div class="timeline-title"><a href="<?php if ($youtubeid == "" && $peertubeid == "") { echo $url; } else { echo WEBSITE_URL ?>/index.php?watch=<?=$youtubeid.$peertubeid; } ?>" <?php if ($youtubeid == "" && $peertubeid == "") { ?> target="_blank" <?php } ?> style="text-decoration: none;"><span style="color:var(--feedbot-gray); font-size: 12px; text-transform: uppercase;"><?=$media_url;?></span><br /><span style="color:var(--feedbot-title); font-weight:bold;"><?=truncate($title,120);?></span></a></div>
    </div>
